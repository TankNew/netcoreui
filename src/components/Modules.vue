  ...
  <section class="container-fluid">
      ...
      <section>
        ...
        <div class="w-50" style="min-width:640px;">
          <b-form @submit="onSubmit" @reset="onReset" v-if="show" autocomplete="off">
            <b-form-group id="exampleInputGroup1" label="标识名称:" label-for="exampleInput1" description="设置一个便于您识别的标识名称，该名称不会显示在任何前端页面中.">
              <b-form-input id="exampleInput1" type="text" v-model="page.name" placeholder="输入一个标识名称" required></b-form-input>
            </b-form-group>
            <b-form-group id="exampleInputGroup2" label="备注:" label-for="exampleInput2">
              <b-form-input id="exampleInput2" type="text" v-model="page.info" placeholder="简单描述这个自定义栏目" required></b-form-input>
            </b-form-group>
            ...
            <b-form-group v-for="(rtem,rid) in page.rows" :key="rid">
              <b-input-group prepend="字段名称" size="sm">
                ...
                <div slot="append" class="ml-1">
                  <button
                    type="button"
                    :class="['btn', 'btn-outline-secondary','btn-sm', 'mr-1', rtem.type==item.name?'active':'']"
                    v-for="(item,index) in basicSections"
                    :key="index"
                    @click="rtem.type=item.name"
                  >{{item.name}}</button>
    ...
    <section v-else>
      ...
      <section>
        ...
        <div class="mb-3">
          <button class="btn btn-outline-secondary mr-3" v-for="(item,index) in basicSections" :key="index">{{item.name}}</button>
        </div>
      </section>
      ...
      <section>
        ...
        <div class="mb-3">
          <button class="btn btn-outline-info mr-3" v-for="(item,index) in sections" :key="index">{{item.name}}</button>
        </div>
      </section>
      ...
      <section>
        ...
        <div class="custom-page-list">
          <b-card-group deck class="mb-3">
            <b-card border-variant="primary" v-for="(item,index) in customSections" :key="index" :header="(index+1)+'.'+item.name" :sub-title="item.createtime" align="center">
...
export default {
  ...
  methods: {
    onSubmit(evt) {
      evt.preventDefault()
      if (!this.isEditMode) this.customSections.push(this.page)
      else {
...
<style scoped>
.custom-page-list .card {
  min-width: calc(25% - 30px);
  max-width: calc(25% - 30px);
  margin-bottom: 10px;
}
.custom-page-tools .card {
  height: 100%;
}
</style>
