    ...
    <section>
      <div ref="ctxMenuContainer">
        ...
        >
          ...
          <li v-if="!menuIsPage" @contextmenu.prevent @click="add(menuData, menuIsPage)">
            <i class="fas color-success fa-plus mr-1"></i>
            增加子模块
          </li>
          <li @contextmenu.prevent @click="del(menuData, menuIndex, menuIsPage, menuArray)">
        ...
        <b-card class="module-body" no-body>
          <b-card-header header-tag="nav">
            <b-nav card-header tabs>
              <b-nav-item :active="currentSection === 1" @click="changeCurrentSection(1)"
                >列表模块</b-nav-item
              >
              <b-nav-item :active="currentSection === 2" @click="changeCurrentSection(2)"
                >设计模块</b-nav-item
              >
              <b-nav-item :active="currentSection === 3" @click="changeCurrentSection(3)"
                >预设模块</b-nav-item
              >
            </b-nav>
          </b-card-header>
                  ...
                  <div v-else>
                    <p class="tips">
                      ...
                      <span class="path">
                        ...
                        <span class="action">{{
                          isUpdate ? module.displayName : parent ? parent.displayName : ``
                        }}</span>
                        {{ !isUpdate ? '子模块' : '' }}
                      </span>
                    ...
                    <b-form @submit.stop.prevent="onSubmit" autocomplete="off">
                      ...
                      <b-input-group size="sm" prepend="图标" class="mb-3">
                      ...
                      <b-form-group
                        label="模块名称:"
                        description="设置一个模块的名称,它有可能显示在内页的二级菜单中"
                      >
                        <b-form-input
                          type="text"
                      ...
                      <b-form-group
                        label="模块别称:"
                        description="设置一个模块的别称,它有可能显示在内页的二级菜单中"
                      >
                        <b-form-input
                          type="text"
                          v-model="module.nickName"
                      ...
                      <b-form-group label="简介:" description="简单的文字描述，不允许换行以及链接.">
                        <b-form-textarea
                          v-model="module.info"
                          placeholder="模块简介"
                          rows="5"
