<template>
  <section class="container-fluid">
    ...
    >
      ...
      <section>
        <file :fileShow="attachShow" :fileCallBack="attachCallBack" @fileClose="attachClose"></file>
        <div class="mb-3 center">
          <div class="news-cover" @click="attachOpen">
            <img :src="getPicture()" />
          </div>
        </div>
        <b-form @submit.stop.prevent="blockSubmit" autocomplete="off" data-vv-scope="form-picture">
          ...
          <b-form-group label="广告链接:" label-for="p-url" description="如果广告无链接，留空即可.">
            <b-form-input
              id="p-url"
              type="text"
    ...
    >
      <b-alert show dismissible> <b>Info:</b> 您可以更改默认分组名称 </b-alert>
      <b-form @submit.stop.prevent="groupModalOk" autocomplete="off" data-vv-scope="form-group">
        <b-form-group
          label="标题:"
          label-for="g-title"
      ...
      <ul>
          ...
          >
            ...
            >
              <div v-if="!item.noActive">
                <i class="fas fa-times fa-delete" @click.stop="blockDelete(item, index)"></i>
                <img :src="item.img" />
              </div>
              <div v-else>
                <span>{{ index + 1 }}</span>
        ...
        <li>
          ...
          >
            ...
            >
              <div v-if="!item.noActive">
                <i class="fas fa-times fa-delete" @click.stop="groupDelete(item)"></i>
...
export default {
  ...
  methods: {
    ...
    getPicture() {
      if (
        this.currentBlock.img !== null &&
        this.currentBlock.img !== '' &&
        this.currentBlock.img !== undefined
      ) {
      ...
      if (await this.validate('form-group')) {
        if (this.value !== undefined && this.value !== null) {
          if (this.currentPageGroupIsUpdate)
            this.groupsCrude[this.currentPageGroup.crudeIndex] = this.currentPageGroup
