  ...
  <section>
    <section v-show="editMode" class="news-edit-editmode">
      ...
      >
        ...
        <b-form-group label="Tags" label-for="tags" description="标签用“,”隔开.">
          <b-form-input
            id="title"
            type="text"
            v-model="form.tags"
        ...
        <div v-if="hasAttach" class="news-img-with-info">
          ...
          <draggable
            ...
            :ghost-class="'ghost'"
          >
            <li v-for="(f, i) in form.pictureWithInfos" :key="i" @click="editPicture(f, i)">
              <img :src="f.picUrl" />
              <i class="fas fa-times fa-lg" @click.stop="attachDelete(i)"></i>
            </li>
            <li class="add" @click="addPicture">
              <span> <i class="fas fa-plus mr-1"></i>添加图片 </span>
            </li>
          </draggable>
        </div>
        <label>正文</label>
      ...
      >
        ...
        >
          ...
          <b-form-group label="标题:" label-for="p-title" description="请填写5-30个字的标题.">
            <b-form-input
              id="p-title"
              type="text"
    ...
    <section v-show="!editMode">
      ...
      <div>
        <b-alert show dismissible>
          <b>Info:</b>
          拖动条目即可排序（稍后上线）。选择快捷工具‘单页显示条目’可改变显示的条目总数。
        </b-alert>
      </div>
          ...
          <b-table-simple hover responsive bordered>
            ...
            >
              <b-tr v-for="(item, index) in items" :key="index">
                ...
                <b-td v-if="hasGroup" class="text-center">{{ item.catalogGroup.displayName }}</b-td>
...
export default {
  ...
  methods: {
    ...
    _edit(item, index, button) {
      ...
      this.form.tags = item.tags ? item.tags.toString() : null
      this.editMode = true
    },
    //新增
    _new() {
    ...
    async onSubmit(evt) {
      ...
      if (await this.validate('form-update')) {
        ...
        this.editRow.tags = this.form.tags.replace(/，/gi, ',').split(',')
