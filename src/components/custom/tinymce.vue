<style scoped>
  ...
  .editor { min-height: 300px; background-color: #fff; position: relative; border: 1px solid #ccc; border-radius: 4px; padding: 1rem; outline: none; }
  #editorToolbar { position: absolute; top: -70px; left: 0; z-index: 2; }
</style>
<template>
  <section :style="{ width: editorWidth+'px',marginTop: editorTop+'px'}" class="editor-container">
    <div ref="editorToolbar" id="editorToolbar"></div>
    <file :fileShow="fileShow" :fileCallBack="fileCallBack" @fileClose="fileClose"></file>
    <div :id="tinymceId" class="editor" @blur="refreshScroll" @keyup="refreshScroll" ref="editorContainer"></div>
  </section>
</template>
<script>
    ...
    methods: {
      _initScroll() {
        ...
        tinymce.init({
          ...
          toolbar: [' undo redo bullist numlist outdent indent alignleft aligncenter alignright alignjustify link | image | picload insertdatetime template',
